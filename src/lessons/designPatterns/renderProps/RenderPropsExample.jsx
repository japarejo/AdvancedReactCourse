...
function CoordinatesReadout({ x, y }) {
  return (
    ...
    >
      ...
      <p style={{ margin: 0, color: '#718096', fontSize: 14 }}>Mueve el cursor sobre el panel</p>
    </div>
  )
}
...
function PointerSpotlight({ x, y, width, height }) {
  ...
  return (
    <>
      ...
      >
        {hasMoved ? `${horizontal}% horizontal, ${vertical}% vertical` : 'Explora el área con el puntero'}
      </div>
    </>
  )
}
  ...
  return (
    ...
    >
      <header style={{ maxWidth: 640 }}>
        ...
        <h2 style={{ margin: '0.25rem 0' }}>Compartiendo lógica de seguimiento del cursor</h2>
        <p style={{ margin: 0, color: '#4a5568' }}>
          <code>MouseTracker</code> encapsula el estado del cursor y lo expone mediante una render prop.
          Dos consumidores diferentes reutilizan la misma lógica y definen su propia interfaz visual.
        </p>
      </header>
      ...
      >
        <MouseTracker label="Lectura directa" render={(position) => <CoordinatesReadout {...position} />} />
        <MouseTracker
